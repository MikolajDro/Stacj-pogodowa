 ...
 *      Author: mikol
 */
#include "epaper_functions.h"

IMAGE_T *Image;
STATION_DATA_T StationData;
uint8_t epaper_mode = NORMAL_MODE;
...
int DrawNormalPressure(IMAGE_T *Image, STATION_DATA_T *station_data)
{
	...
	switch (station_data->presFlag){
		...
		case 2:
			...
			break;
		case -1:
			Paint_DrawBitMap_Paste(gNormalTempTerriblyHot60X60, 62, 10, 60, 60, BLACK);
			Paint_DrawString_EN(2, 52, "Not working", &Font8, WHITE, BLACK);
			break;
		default:
			break;
	}
...
int DrawNormalAirQuality(IMAGE_T *Image, STATION_DATA_T *station_data)
{
	...
	//Set air flag
	if(station_data->air >= 0.f && station_data->air <= 25.f)
		station_data->airFlag = 0;
	else if (station_data->air > 25.f && station_data->air <= 50.f)
		station_data->airFlag = 1;
	else if (station_data->air > 50.f && station_data->air <= 100.f)
		station_data->airFlag = 2;
	else if (station_data->air > 100.f && station_data->air <= 210.f)
		station_data->airFlag = 3;
	else if (station_data->air > 210.f && station_data->air <= 500.f)
	...
	if (station_data->air >= 0.f && station_data->air <= 10.f)
		station_data->xShift = 21;
	else if (station_data->air < 100.f)
...
void StationDataUpdate(struct bme680_field_data data, STATION_DATA_T *station_data)
{
	station_data->air 	= data.gas_resistance;
	station_data->temp	= data.temperature;
	station_data->hum	= data.humidity;
	station_data->pres	= data.pressure;
}
...
EpaperUpdate()
{
	...
	StationDataUpdate(data, &StationData);

	switch(epaper_mode)
	{
		case SIMPLE_MODE:
			DrawSimpleTheme(Image, &StationData);
			break;
		case NORMAL_MODE:
			DrawNormalTheme(Image, &StationData);
			break;
		case ADVANCED_MODE:
//			void DrawAdvancedTheme(Image, StationData);
			HAL_UART_Transmit_IT(&huart1, (uint8_t *)"Advanced mode not supported yet.\r\n", 34);
			break;
		default:
			break;
	}
