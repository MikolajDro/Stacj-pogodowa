...
service_read_buffer(char *buffer, const uint16_t size)
{
	...
	if(buffer[0] == 'A' && buffer[1] == 'T')
	{
		...
		if(buffer[2] == '+')
		{
			...
			{
				uint8_t battery_level = measure_battery_level();
			...
			else if(strncmp((char *)&buffer[3], "VALUES?", strlen("VALUES?")) == 0)
			{
				...
				strcat(TransmitBuffer, ftoa(data.temperature, value_buffor, 1));
				strcat(TransmitBuffer, "*C Hum:");
				strcat(TransmitBuffer, itoa((int )data.humidity, value_buffor, 10));
				strcat(TransmitBuffer, "% Press:");
				strcat(TransmitBuffer, ftoa(data.pressure, value_buffor, 0));
				strcat(TransmitBuffer, "hPa AQI:");
				strcat(TransmitBuffer, itoa((int )data.gas_resistance, value_buffor, 10));
				strcat(TransmitBuffer, "\r\n");
